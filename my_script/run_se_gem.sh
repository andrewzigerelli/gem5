...
# set up trap so we always end up in my_script directory
# if script errors or finishes
function trapFun {
...
CFG=$GEM5_PATH/configs/example/se.py
...
CKPT_DIR=$GEM5_PATH"/"my_script"/"se_checkpoints"/"$BUILD"/"$CORE_NUM"_core"
...
# check if help
if [ "$#" -eq 1 ] && [ "$1" = "--help" ]; then
    echo "./run_se_gem.sh --dry-run to see which would be run"
    echo "./run_se_gem.sh --se-help to see options to gem5 fs.py config"
    echo "./run_se_gem.sh --se-options to pass options to fs.py"
    echo "      e.g. ./run_se_gem.sh --se-options --list-cpu-types"
    exit
fi
if [ "$#" -eq 1 ] && [ "$1" = "--se-help" ]; then
    $GEM_CMD $CFG --help
    exit
fi
if [ "$1" = "--se-options" ]; then
    if [ "$#" -eq 1 ] || [ "$#" -gt 2 ] ; then
        echo "--s-options requires exactly 1 argument following it"
        echo "      e.g. ./run_se_gem.sh --se-options --list-cpu-types"
        exit
    fi
    $GEM_CMD $CFG $2
...
OUT_DIR=$GEM5_PATH"/"my_script"/"output"/se/"$BUILD"/"$CORE_NUM"_core/"$DISK_IMAGE"/"$KERNEL
