...
BENCH_DEBUG_FLAG=Ruby,TLB #RCTStats
...
FULL_CMD=$GEM_CMD" "--outdir=$BENCH_OUT_DIR" "--debug-start=3625891399000" "--debug-flag=$BENCH_DEBUG_FLAG" "--debug-file=$BENCH_DEBUG_FILE" "$CFG" "$BENCH_OPTIONS" "--command-line" '"$CMD_LINE" root="$ROOT"'"
if [ "$1" = "--dry-run" ]; then
    echo $FULL_CMD
    exit
fi
